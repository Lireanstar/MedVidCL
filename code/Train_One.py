...
def train_model(model, fgm,pgd,train_loader,epoch):  # 训练一个epoch
    ...
    for step, (input_ids, attention_mask, token_type_ids,y,vs) in enumerate(tk):
        ...
        with autocast():
            output = model(input_ids, attention_mask=attention_mask,labels=y)
            loss = output.loss
...
from deberta import DebertaV2ForCL,BigBirdForCL
model = AutoModelForSequenceClassification.from_pretrained(CFG['model'],num_labels = 3)  # 模型
