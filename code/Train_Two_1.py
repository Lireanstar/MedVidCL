...
def test_model(model, val_loader,epoch):  # 验证
    ...
    log(['测试损失：{}'.format(str(losses.avg)),'测试准确率：{}'.format(accs.avg),'结束本轮测试'],path)
...
model = AutoModelForSequenceClassification.from_pretrained(CFG['model'],num_labels = 2)  # 模型
