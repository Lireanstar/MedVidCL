...
class DebertaV2ForCL(DebertaV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        video_feature = self.Video_linear(vs)
        video_feature = self.video_affine(video_feature)

        encoder_layer = outputs[0]
        feature = self.cq_concat(video_feature,encoder_layer,attention_mask)
        pooled_output = self.pooler(feature)
...
class BigBirdClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.out_proj(x)
        return x
class BigBirdForCL(BigBirdPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        video_feature = self.video_affine(video_feature)
        sequence_output = outputs[0]
        feature = self.cq_concat(video_feature, sequence_output, attention_mask)
