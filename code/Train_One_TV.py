...
CFG = { #训练的参数配置
    'seed': 1,
    'model': r'microsoft/deberta-v3-large', #预训练模型
...
visual = load_video_features(os.path.join('data', 'features','I3D'),2)
...
def get_data(name):
    ...
    data = []
    for t in tmp:
        t['label'] = {'Medical Non-instructional':0,'Non-medical':1,'Medical Instructional':2}[t['label']]
        t['video_sub_title'] = t['video_sub_title'].replace('[Music]','')
        if t['label'] == 0:
            num_0 += 1
        data.append(t)
    print('label_0 : {}  label_1 : {}'.format(num_0,len(tmp)-num_0))
    return data
train = get_data(r'data/text2/train.json')
valid = get_data(r'data/text2/val.json')
...
def collate_fn(data):
    input_ids, attention_mask, token_type_ids,vs = [], [], [],[]
    for x in data:

        text = tokenizer(x['video_title'],text_pair=x['video_sub_title'], padding='max_length', truncation=True, max_length=CFG['max_len'])
        input_ids.append(text['input_ids'])
        attention_mask.append(text['attention_mask'])
        video = visual[x['video_id']]
        vs.append(video)
    ...
    return input_ids, attention_mask, token_type_ids,label,vs
...
def train_model(model, fgm,pgd,train_loader,epoch):  # 训练一个epoch
    ...
    for step, (input_ids, attention_mask, token_type_ids,y,vs) in enumerate(tk):

        with autocast():
            output = model(input_ids, attention_mask=attention_mask,labels=y,vs=vs)
            loss = output.loss
        ...
        acc = (output.logits.argmax(1) == y).sum().item() / y.size(0)
...
def test_model(model, val_loader,epoch):  # 验证
    ...
    with torch.no_grad():
        ...
        for idx, (input_ids, attention_mask, token_type_ids, y,vs) in enumerate(tk):


            output = model(input_ids, attention_mask=attention_mask,labels=y,vs=vs)
            pred.extend(output.logits)
            loss = output.loss
...
from deberta_audio_attention_hine import DebertaV2ForCL
model = DebertaV2ForCL.from_pretrained(CFG['model'],num_labels = 3)  # 模型
#model.load_state_dict(torch.load(r'spanish_model\baseline_0.8088613047085917_spanish_8.pt',map_location='cpu'))
...
for epoch in range(CFG['epochs']):
    ...
    log_1,l1 = compute_metrics_task1(torch.stack(pred).argmax(1).cpu().numpy(),true_task1)
    log(log_1,path)

    model_name = path+'/{}_{}_model'.format(epoch,str(val_acc))
